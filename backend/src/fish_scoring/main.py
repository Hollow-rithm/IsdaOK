...
def main():
    ...
    fish_img = cv.imread(f"dataset/full/{img_path}_FULL.jpg")
    gill_img = cv.imread(f"dataset/gills/{img_path}_GILLS.jpg")
...
if __name__ == "__main__":
    ...
    #         })
    #         continue
    #     # gill_roi = cv.imread(f"{folder_path}/{file.name}")
    #     # gill_mask = gill_segmenter.get_mask(gill_roi)
    #     gill_feats = gill_features.extract(gill_roi, gill_mask)
    #     body_feats = body_features.extract(body_roi)
    #     eye_feats = eye_features.extract(eye_roi)
    ...
    #         # "gill_score": gill_score["final_score"],
    #     })
    # # df = pd.DataFrame(rows)
    # # print(df.describe())

    # output_path = "csvs/scoring3.csv"
