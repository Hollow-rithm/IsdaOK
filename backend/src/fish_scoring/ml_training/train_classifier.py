...
# ─── 1. Load Data ────────────────────────────────────────────────────────────
data = pd.read_csv("feature_training/species_data.csv")
...
species_names = [label_map[c] for c in model.classes_]
...
plt.tight_layout()
plt.savefig(f"{SAVE_DIR}/species_confusion_matrix.png", dpi=150)
plt.close()
...
plt.tight_layout()
plt.savefig(f"{SAVE_DIR}/species_feature_importance.png", dpi=150)
plt.close()
print("Feature importance chart saved.")

# ─── 9. Save Model + Metadata ────────────────────────────────────────────────
joblib.dump(model, f"{SAVE_DIR}/species_model.pkl")
...
joblib.dump(metadata, f"{SAVE_DIR}/species_model_metadata.pkl")
