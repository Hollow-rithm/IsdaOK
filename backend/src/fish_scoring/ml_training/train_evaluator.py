...
print(classification_report(
    y_test,
    y_pred,
    target_names=[label_map[c] for c in sorted(label_map.keys())]
))

# ─── 6. Confusion Matrix ─────────────────────────────────────────────────────
# Human-readable labels
quality_names = [label_map[c] for c in model.classes_]
...
disp = ConfusionMatrixDisplay(
    confusion_matrix=cm,
    display_labels=quality_names
)
disp.plot(cmap='Blues')
plt.title("Quality Evaluation — Confusion Matrix")
plt.tight_layout()
plt.savefig(f"{SAVE_DIR}/quality_confusion_matrix.png", dpi=150)
plt.close()
...
plt.title("Feature Importance — Quality Evaluator")
plt.tight_layout()
plt.savefig(f"{SAVE_DIR}/quality_feature_importance.png", dpi=150)
plt.close()
print("Feature importance chart saved.")

# ─── 9. Save Model + Metadata ────────────────────────────────────────────────
joblib.dump(model, f"{SAVE_DIR}/quality_model.pkl")
...
joblib.dump(metadata, f"{SAVE_DIR}/quality_model_metadata.pkl")
